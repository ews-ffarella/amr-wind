...
#include "amr-wind/equation_systems/sdr/source_terms/KOmegaSTTerrainSDRSrc.H"
...
KOmegaSTTerrainSDRSrc::KOmegaSTTerrainSDRSrc(const CFDSim& sim)
...
{}
KOmegaSTTerrainSDRSrc::~KOmegaSTTerrainSDRSrc() = default;

void KOmegaSTTerrainSDRSrc::operator()(
    const int lev,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        ...
        if (k == 0) {
            ...
            const amrex::Real sdr =
                ustar / (kappa * z) * std::sqrt(phi_e * phi_m) / std::sqrt(Cmu);
    ...
    if (has_terrain) {
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                ...
                const amrex::Real sdr = ustar / (kappa * z) *
                                        std::sqrt(phi_e * phi_m) /
                                        std::sqrt(Cmu);

                terrainforcing = (sdr - sdr_arr(i, j, k)) / dt;
                amrex::Real bcforcing = 0;
                if (k == 0) {
